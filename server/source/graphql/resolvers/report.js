			...
			try {
				...
				if (dataReport.exist !== '') {
					...
					for (let i = 0; i < dataReport.players.length; i++) {
						...
						//F
						sheetPlayer.getCell(`F${countRowDataPlayer}`).value =
							countInCorrect;
						...
						//G
						sheetPlayer.getCell(`G${countRowDataPlayer}`).value =
							countUnanswered;
					...
					for (let i = 0; i < dataReport.questions.length; i++) {
						...
						for (
							...
							for (
								...
								if (
									z !==
									dataReport.questions[i].detailAllPlayers[j].answered.length -
										1
								) {
									dataAnswer +=
					...
					for (let i = 0; i < dataReport.players.length; i++) {
						...
						sheetQuestion
							.getRow(1)
							.getCell(countColumPlayerQuestion).alignment = {
							vertical: 'middle',
							horizontal: 'center',
							wrapText: true,
						};
